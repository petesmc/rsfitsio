...
*/
    ...
    */
    ...
    {
      if (ffgtcl(fptr, ii + 1, &typecode, &repeat, &width, &status) > 0) 
          goto errstatus;
      printf("%4s %3d %2ld %2ld", tform[ii], typecode, repeat, width);
      if (ffgbcl(fptr, ii + 1, ttype[0], tunit[0], cvalstr, &repeat, &scale,
           &zero, &jnulval, tdisp, &status) > 0)
          goto errstatus;
    ...
    */
    ...
    for (ii = 0; ii < tfields; ii++)
    {
      ...
      printf("%4s %3d %2ld %2ld", tform[ii], typecode, repeat, width);
      if (ffgacl(fptr, ii + 1, ttype[0], tbcol, tunit[0], tform[0], &scale,
           &zero, nulstr, tdisp, &status) > 0) {
        goto errstatus;
      }
