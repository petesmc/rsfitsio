...
use crate::fitscore::{
    ffasfm_safe, ffcmsg_safe, ffgcprll, ffghdt_safe, ffpmsg_slice, ffpmsg_str,
...
pub unsafe extern "C" fn ffgpvsb(
    ...
    unsafe {
        ...
        ffgpvsb_safe(fptr, group, firstelem, nelem, nulval, array, anynul, status)
    }
}
...
pub unsafe extern "C" fn ffg2dsb(
    ...
    unsafe {
        ...
        let array = slice::from_raw_parts_mut(array, (ncols * naxis1 * naxis2) as usize);
...
pub fn ffgsvsb_safe(
    ...
    if naxis < 1 || naxis > 9 {
        int_snprintf!(
            &mut msg,
            FLEN_ERRMSG,
            "NAXIS = {} in call to ffgsvsb is out of range",
            naxis
        );
        ffpmsg_slice(&msg);
        *status = BAD_DIMEN;
        ...
        if trc[ii] < blc[ii] {
            if hdutype == IMAGE_HDU {
                ...
                int_snprintf!(
                    &mut msg,
                    FLEN_ERRMSG,
                    "ffgsvsb: illegal range specified for axis {}",
                    ii + 1
                );
                ffpmsg_slice(&msg);
                *status = BAD_PIX_NUM;
...
pub unsafe extern "C" fn ffggpsb(
    ...
    unsafe {
        ...
        ffggpsb_safe(fptr, group, firstelem, nelem, array, status)
    }
}
...
pub(crate) fn ffgclsb(
    ...
    if strchr_safe(&tform, bb(b'A')).is_some() {
        ...
        repeat = twidth as LONGLONG; /* total no. of chars in the col */
    ...
    } else if tcode == TSBYTE && (tnull > 255 || tnull < 0) {
        nulcheck = NullCheckType::None; /* Impossible null value */
        ...
        if *status > 0 {
            ...
            if hdutype > 0 {
                int_snprintf!(
                    &mut message,
                    FLEN_ERRMSG,
                    "Error reading elements {:.0} thru {:.0} from column {} (ffgclsb).",
                    dtemp + 1.0,
                    dtemp + ntodo as f64,
                    colnum,
                );
                ...
                int_snprintf!(
                    &mut message,
                    FLEN_ERRMSG,
                    "Error reading elements {:.0} thru {:.0} from image (ffgclsb).",
                    dtemp + 1.0,
                    dtemp + ntodo as f64,
                );
            }
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == -128.0 {
            ...
            for ii in 0..(ntodo as usize) {
                output[ii] = ((input[ii]) ^ 0x80) as i8;
            }
        } else if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if input[ii] > i8::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8; /* copy input */
                }
            }
        } else {
            /* must scale the data */
...
pub(crate) fn fffi2s1(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] < i8::MIN.into() {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if input[ii] > i8::MAX.into() {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] == tnull {
                    ...
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if input[ii] > INT32BIT::from(i8::MAX) {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 9223372036854775808. {
            ...
            for ii in 0..(ntodo as usize) {
                ...
                if ulltemp > i8::MAX as ULONGLONG {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = ulltemp as _; //TODO check original code, was a short
                }
            }
        } else if scale == 1.0 && zero == 0.0 {
            /* no scaling */
                    ...
                    *anynull = 1;
                    ...
                    if ulltemp > i8::MAX as ULONGLONG {
                        *status = OVERFLOW_ERR;
                        ...
                        output[ii] = ulltemp as _; //TODO check original code, was a short here
                    }
                }
            }
        } else if scale == 1.0 && zero == 0.0 {
...
pub(crate) fn fffr4s1(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if f64::from(input[ii]) < DSCHAR_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if f64::from(input[ii]) > DSCHAR_MAX {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                ...
                } else if f64::from(input[ii]) < DSCHAR_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if f64::from(input[ii]) > DSCHAR_MAX {
                    *status = OVERFLOW_ERR;
...
mod tests {
    ...
    use crate::fitsio::{
        ASCII_TBL, BAD_DIMEN, BINARY_TBL, BYTE_IMG, LONGLONG, NUM_OVERFLOW, READONLY, SBYTE_IMG,
        fitsfile,
    };
    ...
    fn test_read_primary_array() {
        with_temp_file(|filename| {
            ...
            assert_eq!(status, 0);
            assert_eq!(result, [-128, -50, 0, 50, 127]);
            assert_eq!(anynull, 0);
            fits_close_file(f.take().unwrap(), &mut status);
        });
    }
    ...
    fn test_read_2d_array() {
        with_temp_file(|filename| {
            ...
            fits_write_2d_sbyt(f.as_deref_mut().unwrap(), 1, 3, 3, 2, &data, &mut status);
            fits_close_file(f.take().unwrap(), &mut status);
    ...
    fn test_read_subsection() {
        with_temp_file(|filename| {
            ...
            fits_create_file(&mut f, &name, &mut status);
            fits_write_imghdr(f.as_deref_mut().unwrap(), SBYTE_IMG, 2, &naxes, &mut status);
            fits_write_img_sbyt(f.as_deref_mut().unwrap(), 1, 1, 16, &data, &mut status);
            fits_close_file(f.take().unwrap(), &mut status);
    ...
    fn test_read_binary_table_column() {
        with_temp_file(|filename| {
            ...
            let mut f = make_table(&name, BINARY_TBL, "SBCOL", "1S", 3, &mut status);
    ...
    fn test_read_from_float_column() {
        with_temp_file(|filename| {
            let mut status = 0;
            let name = to_buf(filename);
            let data: [f32; 3] = [-128.0, 0.0, 127.0];
    ...
    fn test_read_from_ascii_table() {
        with_temp_file(|filename| {
            ...
            let mut f = make_table(&name, ASCII_TBL, "NUMCOL", "F6.1", 3, &mut status);
            fits_write_col_dbl(f.as_deref_mut().unwrap(), 1, 1, 1, 3, &data, &mut status);
            fits_close_file(f.take().unwrap(), &mut status);
    ...
    fn test_read_multirow() {
        with_temp_file(|filename| {
            ...
            let mut f = make_table(&name, BINARY_TBL, "SBCOL", "3S", 3, &mut status);
            fits_write_col_sbyt(f.as_deref_mut().unwrap(), 1, 1, 1, 9, &data, &mut status);
            fits_close_file(f.take().unwrap(), &mut status);
    ...
    fn test_read_variable_length_array() {
        with_temp_file(|filename| {
            let mut status = 0;
            let name = to_buf(filename);
            let data1: [i8; 3] = [10, 20, 30];
            let data2: [i8; 2] = [40, 50];

            let mut f = make_table(&name, BINARY_TBL, "VARSB", "1PS", 2, &mut status);
    ...
    fn test_read_group_parameters() {
        with_temp_file(|filename| {
            ...
            let pdata: [i8; 2] = [10, 20];
            let idata: [i8; 4] = [1, 2, 3, 4];
            ...
            fits_write_grppar_sbyt(f.as_deref_mut().unwrap(), 1, 1, 2, &pdata, &mut status);
            fits_write_img_sbyt(f.as_deref_mut().unwrap(), 1, 1, 4, &idata, &mut status);
            fits_close_file(f.take().unwrap(), &mut status);

            fits_open_file(&mut f, &name, READONLY, &mut status);
            let mut presult = [0i8; 2];
