...
pub fn ffgpvsb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TSBYTE, firstelem, nelem, nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgpfsb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TSBYTE, firstelem, nelem,          nullcheck, None, cast_slice_mut(array), nularray, anynul, status);
        return *status;
    }
...
pub fn ffg3dsb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        //fits_read_compressed_img(fptr, TSBYTE, fpixel, lpixel, inc,            nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgsvsb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        // fits_read_compressed_img(fptr, TSBYTE, blcll, trcll, inc,            nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgsfsb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        // fits_read_compressed_img(fptr, TSBYTE, blcll, trcll, inc,     nullcheck, None, cast_slice_mut(array), flagval, anynul, status);
        return *status;
    }
...
pub(crate) fn fffi2s1(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] < i8::MIN as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if input[ii] > i8::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] == tnull {
                    ...
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if input[ii] > i8::MAX as INT32BIT {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
        ...
        if scale == 1.0 && zero == 9223372036854775808. {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] < 0 {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if input[ii] > i8::MAX as LONGLONG {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
...
pub(crate) fn fffr4s1(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if (input[ii] as f64) < DSCHAR_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if (input[ii] as f64) > DSCHAR_MAX {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MAX;
                } else {
                    output[ii] = input[ii] as i8;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                ...
                } else if (input[ii] as f64) < DSCHAR_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = i8::MIN;
                } else if (input[ii] as f64) > DSCHAR_MAX {
                    *status = OVERFLOW_ERR;
