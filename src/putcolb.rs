...
pub fn ffpprb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_write_compressed_pixels(fptr, TBYTE, firstelem, nelem,    0, array, &nullvalue, status);
        return *status;
    }
...
pub fn ffppnb_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_write_compressed_pixels(fptr, TBYTE, firstelem, nelem, 1, array, &nullvalue, status);
        return *status;
    }
...
pub unsafe extern "C" fn ffpssb(
    ...
    unsafe {
        ...
        // TODO: Probably need to take first `naxis` number of items and only sum up those
