...
pub fn ffgpvi_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TSHORT, firstelem, nelem, nullcheck, &nullvalue, array, NULL, anynul, status);
        return *status;
    }
...
pub fn ffgpfi_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TSHORT, firstelem, nelem,          nullcheck, NULL, array, nularray, anynul, status);
        return *status;
    }
...
pub fn ffg3di_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        //fits_read_compressed_img(fptr, TSHORT, fpixel, lpixel, inc,            nullcheck, &nullvalue, array, NULL, anynul, status);
        return *status;
    }
...
pub fn ffgsvi_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        // fits_read_compressed_img(fptr, TSHORT, blcll, trcll, inc,            nullcheck, &nullvalue, array, NULL, anynul, status);
        return *status;
    }
...
pub fn ffgsfi_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        // fits_read_compressed_img(fptr, TSHORT, blcll, trcll, inc,     nullcheck, NULL, array, flagval, anynul, status);
        return *status;
    }
...
pub(crate) fn fffi4i2(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] < c_short::MIN as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MIN;
                } else if input[ii] > c_short::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MAX;
                } else {
                    output[ii] = input[ii] as _;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] == tnull {
                    ...
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MIN;
                } else if input[ii] > c_short::MAX as INT32BIT {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MAX;
                } else {
                    output[ii] = input[ii] as _;
...
pub(crate) fn fffr4i2(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if (input[ii] as f64) < DSHRT_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MIN;
                } else if (input[ii] as f64) > DSHRT_MAX {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MAX;
                } else {
                    output[ii] = input[ii] as _;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                ...
                } else if (input[ii] as f64) < DSHRT_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_short::MIN;
                } else if (input[ii] as f64) > DSHRT_MAX {
                    *status = OVERFLOW_ERR;
