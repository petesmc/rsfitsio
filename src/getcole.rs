...
pub(crate) fn fffr4r4_inplace(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            /* must scale the data */
            for ii in 0..(ntodo as usize) {
                input[ii] = ((input[ii] as f64) * scale + zero) as _;
            }
        }
    } else {
        /* must check for null values */
...
pub(crate) fn fffr8r4(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if input[ii] < -f32::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = -f32::MAX;
                } else if input[ii] > f32::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = f32::MAX;
                } else {
                    output[ii] = input[ii] as _; /* copy input to output */
            ...
            for ii in 0..(ntodo as usize) {
                ...
                if dvalue < -f32::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = -f32::MAX;
                } else if dvalue > f32::MAX as _ {
                    *status = OVERFLOW_ERR;
                    output[ii] = f32::MAX;
                } else {
                ...
                if 0 != iret {
                    ...
                    if iret == 1 {
                        ...
                        *anynull = 1;
                        ...
                        /* it's an underflow */
                        if zero < -f32::MAX as _ {
                            *status = OVERFLOW_ERR;
                            output[ii] = -f32::MAX as _;
                        } else if zero > f32::MAX as _ {
                            *status = OVERFLOW_ERR;
                            output[ii] = f32::MAX;
                        } else {
                            output[ii] = zero as _;
