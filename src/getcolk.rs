...
pub fn ffgpvk_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TINT, firstelem, nelem, nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgpfk_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_read_compressed_pixels(fptr, TINT, firstelem, nelem,          nullcheck, None, cast_slice_mut(array), nularray, anynul, status);
        return *status;
    }
...
pub fn ffg3dk_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        //fits_read_compressed_img(fptr, TINT, fpixel, lpixel, inc,            nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgsvk_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        nullvalue = nulval; /* set local variable */
        todo!();
        // fits_read_compressed_img(fptr, TINT, blcll, trcll, inc,            nullcheck, &nullvalue, cast_slice_mut(array), None, anynul, status);
        return *status;
    }
...
pub fn ffgsfk_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        // fits_read_compressed_img(fptr, TINT, blcll, trcll, inc,     nullcheck, None, cast_slice_mut(array), flagval, anynul, status);
        return *status;
    }
        ...
        if scale == 1.0 && zero == 9223372036854775808. {
            ...
            for ii in 0..(ntodo as usize) {
                if input[ii] < 0 {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_int::MIN;
                } else if input[ii] > c_int::MAX as LONGLONG {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_int::MAX;
                } else {
                    output[ii] = input[ii] as _;
...
pub(crate) fn fffr4int(
    ...
    if nullcheck == NullCheckType::None {
        ...
        if scale == 1.0 && zero == 0.0 {
            /* no scaling */
            for ii in 0..(ntodo as usize) {
                if (input[ii] as f64) < DINT_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_int::MIN;
                } else if (input[ii] as f64) > DINT_MAX {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_int::MAX;
                } else {
                    output[ii] = input[ii] as _;
        ...
        if scale == 1.0 && zero == 0.0 {
            ...
            for ii in 0..(ntodo as usize) {
                ...
                } else if (input[ii] as f64) < DINT_MIN {
                    *status = OVERFLOW_ERR;
                    output[ii] = c_int::MIN;
                } else if (input[ii] as f64) > DINT_MAX {
                    *status = OVERFLOW_ERR;
