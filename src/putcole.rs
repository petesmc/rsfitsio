...
pub fn ffppre_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        //fits_write_compressed_pixels(fptr, TFLOAT, firstelem, nelem,  0, array, &nullvalue, status);
        return *status;
    }
...
pub(crate) fn ffppne_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        // fits_write_compressed_pixels(fptr, TFLOAT, firstelem, nelem,            1, array, &nullvalue, status);
        return *status;
    }
...
pub(crate) fn ffp3de_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        ...
        todo!();
        // fits_write_compressed_img(fptr, TFLOAT, fpixel, lpixel,        0,  array, NULL, status);
...
pub fn ffpsse_safe(
    ...
    if fits_is_compressed_image_safe(fptr, status) > 0 {
        /* this is a compressed image in a binary table */
        todo!();
        //fits_write_compressed_img(fptr, TFLOAT, fpixel, lpixel,            0,  array, None, status);
...
pub(crate) fn ffr4fi8(
    ...
    if scale == 1.0 && zero == 9223372036854775808. {
        ...
        for ii in 0..(ntodo as usize) {
            ...
            } else if (input[ii] as f64) > 2.0 * DLONGLONG_MAX {
                *status = OVERFLOW_ERR;
