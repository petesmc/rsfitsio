...
static mut shared_lt: Option<Vec<SHARED_LTAB>> = None; /* local table pointer */
static mut shared_gt: &mut [SHARED_GTAB] = &mut []; /* global table pointer */
static mut shared_create_mode: c_int = 0666; /* permission flags for created objects */
static mut shared_debug: bool = true; /* simple debugging tool, set to 0 to disable messages */
...
unsafe fn shared_clear_entry(idx: usize) -> c_int /* unconditionally clear entry */ {
    ...
    shared_gt[idx as usize].key = SHARED_INVALID; /* clear entries in global table */
    shared_gt[idx as usize].handle = SHARED_INVALID;
    shared_gt[idx as usize].sem = SHARED_INVALID;
    shared_gt[idx as usize].semkey = SHARED_INVALID;
    shared_gt[idx as usize].nprocdebug = 0;
    shared_gt[idx as usize].size = 0;
    shared_gt[idx as usize].attr = 0;
...
unsafe fn shared_destroy_entry(idx: usize) -> c_int /* unconditionally destroy sema & shseg and clear entry */
{
    ...
    if SHARED_INVALID != shared_gt[idx as usize].sem {
        r = semctl(shared_gt[idx as usize].sem, 0, IPC_RMID, filler); /* destroy semaphore */
    }
    if SHARED_INVALID != shared_gt[idx as usize].handle {
        r2 = shmctl(shared_gt[idx as usize].handle, IPC_RMID, std::ptr::null_mut()); /* destroy shared memory segment */
    }
    if SHARED_OK == r {
...
unsafe fn shared_cleanup() /* this must (should) be called during exit/abort */
{
    ...
    if let Some(slt) = shared_lt {
        ...
        shared_lt = None; /* free local table */
    }
        ...
        if SHARED_INVALID != shared_fd {
            ...
            if -1 != fcntl(shared_fd, F_SETLK, &mut flk) {
                ...
                if !segmentspresent {
                    ...
                    if 0 == shmctl(shared_gt_h, IPC_STAT, &mut ds) {
                        /* get number of processes attached to table */

                        let ds: semid_ds = ds.assume_init();
...
unsafe fn shared_init(debug_msgs: c_int) -> c_int /* initialize shared memory stuff, you have to call this routine once */
{
    ...
    if SHARED_INVALID == shared_gt_h {
        ...
        if SHARED_INVALID == shared_gt_h {
            ...
            shared_gt = unsafe { shmat(shared_gt_h, ptr::null(), 0) } as *mut SHARED_GTAB; /* attach segment */
    ...
    if shared_lt.is_none() {
        ...
        for i in 0..shared_maxseg as usize {
            v[i as usize].p = None; /* not mapped */
            v[i as usize].tcnt = 0; /* unused (or zero threads using this seg) */
            v[i as usize].lkcnt = 0; /* segment is unlocked */
            v[i as usize].seekpos = 0; /* r/w pointer at the beginning of file */
        }
...
unsafe fn shared_delta_process(sem: c_int, delta: c_int) -> c_int /* change number of processes hanging on segment */
{
    ...
    return if -1 == semop(sem, &mut sb, 1) {
        SHARED_IPCERR
    } else {
        SHARED_OK
    };
...
unsafe fn shared_malloc(size: c_long, mode: c_int, newhandle: c_int) -> c_int /* return idx or SHARED_INVALID */
{
    ...
    loop {
        ...
        if let Some(slt) = shared_lt.as_deref_mut() {
            if mode & SHARED_RESIZE != 0 {
                if unsafe { shmdt(bp) != 0 } {
                    r = SHARED_IPCERR; /* if segment is resizable, then detach segment */
                }
                slt[idx as usize].p = None;
            } else {
                slt[idx as usize].p = bp;
            }

            slt[idx as usize].tcnt = 1; /* one thread using segment */
            slt[idx as usize].lkcnt = 0; /* no locks at the moment */
            slt[idx as usize].seekpos = 0; /* r/w pointer positioned at beg of block */
...
unsafe fn shared_attach(idx: usize) -> c_int {
    ...
    /* try attach process */
    if shared_attach_process(shared_gt[idx as usize].sem) != 0 {
        shmdt(shared_lt[idx as usize].p); /* cannot attach process, detach everything */
        shared_lt[idx as usize].p = None;
        shared_demux(idx, SHARED_RDWRITE);
        return SHARED_BADARG;
    }

    shared_lt[idx as usize].tcnt += 1; /* one more thread is using segment */

    /* if resizeable, detach and return special pointer */
    if shared_gt[idx as usize].attr as c_int & SHARED_RESIZE != 0 {
        if shmdt(shared_lt[idx as usize].p) != 0 {
            r = SHARED_IPCERR; /* if segment is resizable, then detach segment */
        }
        shared_lt[idx as usize].p = None;
    }
...
unsafe fn shared_check_locked_index(idx: usize) -> c_int /* verify that given idx is valid */ {
    ...
    if shared_lt[idx as usize].p.is_null() {
        return SHARED_BADARG; /* NULL pointer, not attached ?? */
    }

    if 0 == shared_lt[idx as usize].lkcnt {
        return SHARED_BADARG; /* not locked ?? */
    }

    if (SHARED_ID_0 != (shared_lt[idx as usize].p).s.ID[0])
        || (SHARED_ID_1 != (shared_lt[idx as usize].p).s.ID[1])
        || (BLOCK_SHARED != (shared_lt[idx as usize].p).s.tflag)
    {
...
unsafe fn shared_map(idx: usize) -> c_int /* map all tables for given idx, check for validity */ {
    ...
    if SHARED_INVALID == shared_gt[idx as usize].key {
        return SHARED_BADARG;
    }

    (h = shmget(shared_gt[idx as usize].key, 1, shared_create_mode));
    if SHARED_INVALID == h {
        return SHARED_BADARG;
    }
    ...
    if (SHARED_ID_0 != bp.s.ID[0])
        ...
        || (h != shared_gt[idx as usize].handle)
    {
    ...
    if shared_gt[idx as usize].sem != semget(shared_gt[idx as usize].semkey, 1, shared_create_mode) {
        shmdt(bp); /* cannot attach semaphore, detach everything */
        return SHARED_BADARG;
    }

    shared_lt[idx as usize].p = bp; /* store pointer to shmem data */
    return SHARED_OK;
}
...
unsafe fn shared_validate(idx: usize, mode: c_int) -> c_int /* use intrnally inside crit.sect !!! */
{
    ...
    if shared_lt[idx as usize].p.is_none() {
        (r = shared_map(idx));
        if SHARED_OK != r {
...
unsafe fn shared_realloc(idx: usize, newsize: c_int) -> SHARED_P /* realloc shared memory segment */
{
    ...
    if 0 == (shared_gt[idx as usize].attr as c_int & SHARED_RESIZE) {
        return ptr::null();
    }

    if -1 != shared_lt[idx as usize].lkcnt {
        return ptr::null(); /* check for RW lock */
    }

    if shared_adjust_size(shared_gt[idx as usize].size) == shared_adjust_size(newsize) {
        shared_gt[idx as usize].size = newsize;

        return ((shared_lt[idx as usize].p) + 1) as SHARED_P;
    }
    ...
    loop {
        ...
        *bp = *(shared_lt[idx as usize].p); /* copy header, then data */
        transfersize = if newsize < shared_gt[idx as usize].size {
            newsize.into()
        } else {
            shared_gt[idx as usize].size
        };

        if transfersize > 0 {
            memcpy(
                bp + 1,
                (shared_lt[idx as usize].p) + 1,
        ...
        if shmctl(shared_gt[idx as usize].handle, IPC_RMID, std::ptr::null_mut()) != 0 {
            if SHARED_OK == r {
                r = SHARED_IPCERR; /* destroy old shared memory segment */
            }
        }

        shared_gt[idx as usize].size = newsize; /* signal new size */
        shared_gt[idx as usize].handle = h; /* signal new handle */
        shared_gt[idx as usize].key = key; /* signal new key */
        shared_lt[idx as usize].p = bp;
        break;
    }
    return (bp + 1) as SHARED_P;
}
...
unsafe fn shared_free(idx: usize) -> c_int /* detach segment, if last process & !PERSIST, destroy segment */
{
    ...
    (r = shared_detach_process(shared_gt[idx as usize].sem));
    ...
    shared_lt[idx as usize].tcnt -= 1; /* update number of threads using segment */

    if shared_lt[idx as usize].tcnt > 0 {
    ...
    shared_lt[idx as usize].p = None; /* clear entry in local table */
    shared_lt[idx as usize].seekpos = 0; /* r/w pointer positioned at beg of block */
    (cnt = shared_process_count(shared_gt[idx as usize].sem));
    ...
    if (0 == cnt) && (0 == (shared_gt[idx as usize].attr as c_int & SHARED_PERSIST)) {
...
fn shared_lock(idx: usize, mode: c_int) -> SHARED_P /* lock given segment for exclusive access */ {
    ...
    if (mode & SHARED_RDWRITE) != 0 {
        shared_lt[idx as usize].lkcnt = -1;

        shared_gt[idx as usize].nprocdebug += 1;
    } else {
        shared_lt[idx as usize].lkcnt += 1;
    }
...
unsafe fn shared_unlock(idx: usize) -> c_int /* unlock given segment, assumes seg is locked !! */ {
    ...
    if shared_lt[idx as usize].lkcnt > 0 {
        shared_lt[idx as usize].lkcnt -= 1; /* unlock read lock */
        mode = SHARED_RDONLY;
    } else {
        shared_lt[idx as usize].lkcnt = 0; /* unlock write lock */
        shared_gt[idx as usize].nprocdebug -= 1;
        mode = SHARED_RDWRITE;
    }

    if 0 == shared_lt[idx as usize].lkcnt {
        if shared_gt[idx as usize].attr & SHARED_RESIZE !=0 {
            if shmdt(shared_lt[idx as usize].p) {
                r = SHARED_IPCERR; /* segment is resizable, then detach segment */
            }
            shared_lt[idx as usize].p = None; /* signal detachment in local table */
        }
    }
...
unsafe fn shared_set_attr(idx: usize, newattr: c_int) -> c_int /* get the attributes of the shared memory segment */
{
    ...
    r = shared_gt[idx as usize].attr as c_int;
    shared_gt[idx as usize].attr = newattr as u8;
    return r;
}
    ...
    for i in 0..shared_maxseg {
        ...
        match shared_mux(i.try_into().unwrap(), SHARED_NOWAIT | SHARED_RDONLY) {
            SHARED_AGAIN => {
                print!(
                    "!{:3} {:08x} {:4}  {:8}",
                    i, shared_gt[i as usize].key as c_ulong, shared_gt[i as usize].nprocdebug, shared_gt[i as usize].size
                );
...
unsafe fn shared_getaddr(id: c_int, address: &mut *mut c_char) -> c_int {
    ...
    let slt = shared_lt.as_deref_mut().unwrap();

    strcpy_safe(&mut segname, cs!(c"h"));
    int_snprintf!(segname[1..].as_mut_ptr(), 9, "{}", id);

    if smem_open(segname.as_ptr(), 0, &mut i) != 0 {
        return SHARED_BADARG;
    }

    *address = (((slt[i as usize].p + 1) as *const DAL_SHM_SEGHEAD) + 1) as *const c_char;
    /*  smem_close(i); */
    return SHARED_OK;
}
...
pub(crate) fn smem_create(filename: *const c_char, driverhandle: &mut c_int) -> c_int {
    ...
    (sp = shared_lock(h.try_into().unwrap(), SHARED_RDWRITE) as *mut DAL_SHM_SEGHEAD);
    if sp.is_null() {
        shared_free(h.try_into().unwrap());
        return SHARED_BADARG;
    }
...
pub(crate) fn smem_close(driverhandle: usize) -> c_int {
    ...
    (r = unsafe { shared_unlock(driverhandle) });
    if SHARED_OK != r {
        return r;
    }
    return unsafe { shared_free(driverhandle) };
}
...
pub(crate) unsafe fn smem_size(driverhandle: usize, size: *mut LONGLONG) -> c_int {
    ...
    *size = (shared_gt[driverhandle as usize].size - size_of::<DAL_SHM_SEGHEAD>()) as LONGLONG;
    return 0;
}

pub(crate) fn smem_flush(driverhandle: usize) -> c_int {
    if unsafe { shared_check_locked_index(driverhandle) } != 0 {
        return SHARED_INVALID;
    }
    return 0;
}
...
pub(crate) fn smem_read(driverhandle: usize, buffer: *mut c_void, nbytes: usize) -> c_int {
    ...
    if (shared_lt[driverhandle as usize].seekpos + nbytes as c_long) > shared_gt[driverhandle as usize].size {
        return SHARED_BADARG; /* read beyond EOF */
    }

    memcpy(
        buffer,
        ((((shared_lt[driverhandle as usize].p + 1) as *const DAL_SHM_SEGHEAD) + 1) as *const c_char)
...
pub(crate) fn smem_write(driverhandle: usize, buffer: *mut c_void, nbytes: usize) -> c_int {
    ...
    if (shared_lt[driverhandle as usize].seekpos + nbytes as c_long) as c_ulong
        > (shared_gt[driverhandle as usize].size - size_of::<DAL_SHM_SEGHEAD>()) as c_ulong
    {
        /* need to realloc shmem */
        if shared_realloc(
            driverhandle,
            shared_lt[driverhandle as usize].seekpos + nbytes + size_of::<DAL_SHM_SEGHEAD>(),
        )
        .is_null()
        {
            return SHARED_NOMEM;
