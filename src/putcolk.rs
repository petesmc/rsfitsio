...
pub fn ffpclk_safe(
    ...
    if c_int::BITS == c_short::BITS {
        ...
        */
        ...
        let mut maxelem = maxelem2 as LONGLONG;
    ...
    if scale == 1.0 && zero == 0.0 {
        for ii in 0..(ntodo as usize) {
            if input[ii] < 0 {
                *status = OVERFLOW_ERR;
                output[ii] = 0;
            } else if input[ii] > u8::MAX as c_int {
                *status = OVERFLOW_ERR;
                output[ii] = u8::MAX;
            } else {
...
pub(crate) fn ffintfi2(
    ...
    if scale == 1.0 && zero == 0.0 {
        for ii in 0..(ntodo as usize) {
            if input[ii] > c_short::MAX as c_int {
                *status = OVERFLOW_ERR;
