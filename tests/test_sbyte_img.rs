    ...
    fn test_sbyte_write_sbyte_read() {
        with_temp_file(|filename| {
            ...
            unsafe {
                ...
                fits_create_img(fptr.as_mut().unwrap(), SBYTE_IMG, naxis as c_int, &naxes, &mut status);
                fits_write_img(fptr.as_mut().unwrap(), TSBYTE, 1, nelements as LONGLONG,
    ...
    fn test_sbyte_write_byte_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SBYTE with negatives → BYTE");
                
                // Verify clamping: negatives → 0, positives unchanged
                for i in 0..nelements {
                    let expected = if write_data[i] < 0 { 0 } else { write_data[i] as c_uchar };
    ...
    fn test_sbyte_write_ushort_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SBYTE with negatives → USHORT");
                
                // Verify clamping: negatives → 0, positives unchanged
                for i in 0..nelements {
                    let expected = if write_data[i] < 0 { 0 } else { write_data[i] as u16 };
                    assert_eq!(read_data[i], expected,
    ...
    fn test_sbyte_write_short_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                for i in 0..nelements {
                    assert_eq!(write_data[i] as c_short, read_data[i],
    ...
    fn test_sbyte_write_longlong_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "SBYTE → LONGLONG should succeed without overflow");
    ...
    fn test_sbyte_write_ulong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SBYTE with negatives → ULONG");
    ...
    fn test_sbyte_write_ulonglong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SBYTE with negatives → ULONGLONG");
