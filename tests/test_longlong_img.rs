...
mod tests {
    ...
    fn test_longlong_write_longlong_read() {
        with_temp_file(|filename| {
            ...
            unsafe {
                ...
                fits_create_img(fptr.as_mut().unwrap(), LONGLONG_IMG, naxis as c_int, &naxes, &mut status);
                fits_write_img(fptr.as_mut().unwrap(), TLONGLONG, 1, nelements as LONGLONG,
    ...
    fn test_longlong_write_byte_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG → BYTE");
    ...
    fn test_longlong_write_long_read_conversion() {
        ...
        with_temp_file(|filename| {
            ...
            write_data[0] = if c_long::BITS == 32 { c_long::MIN as c_longlong - 1} else { c_long::MIN as c_longlong}; // Underflow LONG (LONG min - 1)
            write_data[1] = c_long::MIN as c_longlong; // Min LONG
            write_data[2] = 0;           // Valid
            write_data[3] = c_long::MAX as c_longlong;  // Max LONG
            write_data[4] =if c_long::BITS == 32 { c_long::MAX as c_longlong +1} else { c_long::MAX as c_longlong};  // Overflow LONG (LONG max + 1)
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "LONGLONG → LONG conversion should succeed without overflow reporting");
                
                // Print actual conversion values to understand behavior
                println!("Actual LONGLONG → LONG conversion values:");
                for i in 0..6 {
                    println!("  write_data[{}] = {} → read_data[{}] = {}", i, write_data[i], i, read_data[i]);
                }
                
                // Test specific conversions (no clamping, direct conversion)
                assert_eq!(read_data[0], c_long::MIN as c_longlong, "Min LONG should convert exactly");
                assert_eq!(read_data[1], c_long::MIN as c_longlong, "Min LONG should convert exactly");
                assert_eq!(read_data[2], 0, "Zero should convert exactly");
                assert_eq!(read_data[3], c_long::MAX as c_longlong, "Max LONG should convert exactly");
                assert_eq!(read_data[4], c_long::MAX as c_longlong, "Max LONG + 1 should clamp");
                assert_eq!(read_data[5], if c_long::BITS == 32 { c_long::MAX as c_longlong } else { 2^47 }, "Large overflow should clamp");
    ...
    fn test_longlong_write_double_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "LONGLONG → DOUBLE should succeed without overflow");
    ...
    fn test_longlong_write_short_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG → SHORT");
    ...
    fn test_longlong_write_ushort_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG → USHORT");
    ...
    fn test_longlong_write_float_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "LONGLONG → FLOAT should succeed without overflow");
    ...
    fn test_longlong_write_sbyte_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG → SBYTE");
    ...
    fn test_longlong_write_ulong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG with negatives → ULONG");
    ...
    fn test_longlong_write_ulonglong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for LONGLONG with negatives → ULONGLONG");
