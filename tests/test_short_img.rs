    ...
    fn test_short_write_short_read() {
        with_temp_file(|filename| {
            ...
            unsafe {
                ...
                fits_create_img(fptr.as_mut().unwrap(), SHORT_IMG, naxis as c_int, &naxes, &mut status);
                fits_write_img(fptr.as_mut().unwrap(), TSHORT, 1, nelements as LONGLONG,
    ...
    fn test_short_write_byte_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) when reading SHORT with negatives as BYTE");
    ...
    fn test_short_write_byte_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "Should succeed reading SHORT in BYTE range as TBYTE");
                
                // Verify data matches
                for i in 0..nelements {
                    assert_eq!(write_data[i] as c_uchar, read_data[i],
    ...
    fn test_short_write_byte_read_detailed_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for mixed-range data");
                
                // Check specific values - CFITSIO clamps overflowed values but reports NUM_OVERFLOW
                assert_eq!(read_data[0], 0, "Underflow value (-1) should be clamped to 0");
                assert_eq!(read_data[1], 0, "Underflow value (-100) should be clamped to 0");
                assert_eq!(read_data[5], 255, "Overflow value (256) should be clamped to 255");  
                assert_eq!(read_data[6], 255, "Overflow value (1000) should be clamped to 255");
                
                // Check that valid values are properly converted
                assert_eq!(read_data[2], 0, "Valid value (0) should be converted correctly");
                assert_eq!(read_data[3], 127, "Valid value (127) should be converted correctly");
                assert_eq!(read_data[4], 255, "Valid value (255) should be converted correctly");
                ...
                println!("Read data verification complete - overflowed values clamped, NUM_OVERFLOW reported");
            }
    ...
    fn test_short_write_ushort_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SHORT → USHORT with negatives");
    ...
    fn test_short_write_longlong_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "SHORT → LONGLONG should succeed without overflow");
    ...
    fn test_short_write_ulong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SHORT → ULONG with negatives");
    ...
    fn test_short_write_ulonglong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for SHORT → ULONGLONG with negatives");
