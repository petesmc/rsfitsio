...
mod tests {
    ...
    fn test_byte_write_short_read() {
        with_temp_file(|filename| {
            ...
            unsafe {
                fits_create_file(&mut fptr, cast_slice(filename_cstr.to_bytes_with_nul()), &mut status);
                fits_create_img(fptr.as_mut().unwrap(), BYTE_IMG, naxis as c_int, &naxes, &mut status);
                fits_write_img(fptr.as_mut().unwrap(), TBYTE, 1, nelements as LONGLONG,
                              cast_slice(&write_data), &mut status);
                fits_close_file(fptr.take().unwrap(), &mut status);
            }
    ...
    fn test_byte_write_ushort_read() {
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                for i in 0..nelements {
                    assert_eq!(write_data[i] as u16, read_data[i],
                             "Data mismatch at index {}", i);
                }
            }
    ...
    fn test_byte_write_sbyte_read() {
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                for i in 0..nelements {
                    let expected = if write_data[i] > 127 { 127 } else { write_data[i] as i8 };
                    assert_eq!(expected, read_data[i],
