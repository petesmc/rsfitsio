    ...
    fn test_float_write_float_read() {
        with_temp_file(|filename| {
            ...
            unsafe {
                ...
                fits_create_img(fptr.as_mut().unwrap(), FLOAT_IMG, naxis as c_int, &naxes, &mut status);
                fits_write_img(fptr.as_mut().unwrap(), TFLOAT, 1, nelements as LONGLONG,
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                for i in 0..nelements {
                    assert!(floats_close_f32(write_data[i], read_data[i]),
    ...
    fn test_float_write_ushort_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for FLOAT → USHORT");
    ...
    fn test_float_write_long_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                // Verify specific conversions (truncation + clamping)
                assert_eq!(read_data[0], c_long::MIN, "Underflow should clamp to LONG min");
                ...
                assert_eq!(read_data[6], c_long::MAX, "Overflow should clamp to LONG max");
                assert_eq!(read_data[7], c_long::MAX, "Large overflow should clamp to LONG max");
            }
    ...
    fn test_float_write_longlong_read_success() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 0, "FLOAT → LONGLONG should succeed without overflow");
    ...
    fn test_float_write_ulonglong_read_overflow() {
        ...
        with_temp_file(|filename| {
            ...
            if let Some(ref mut fptr_box) = fptr {
                ...
                assert_eq!(status, 412, "Expected NUM_OVERFLOW (412) for FLOAT with negatives → ULONGLONG");
